...
TotalUnits is a type synonym for (Set T.Text) and thus an SCalendar is only a pair of
...
where
 - UTCTime: The starting point of time of the calendar, for example, it could be
   a calendar starting from 2016-05-04 00:00:00utc.
...
So if everything is ok, this function Just returns an new Calendar which is suitable for
the given NumDays. A new Calendar is one which has neither reservations nor cacellations.
...
where we want to check availability. Naturally, this function returns a bool if that
amount of resource is available.
Note that here we are just concerned with the amount of resource and whether there is
...
which  will return an SCalendar only with the reservations that pass the
isReservAvailable test. Here we must ake into account that reservations will be
inserted in the order they are in the input list. So, if a reservation conflicts
with the ones that have been alredy inserted, it will not be included in the tree.
...
period of time.
Be careful with this operation because there is no constraint over the period you are
...
conflicts while making availabilty checks, reservations, cacellations, and so on.
...
sort of calendar and resource availabilty management!!!!!!!
...
The author of the algorith is Rayrole, Martin: "Method and device or arrangement for the management of a resource schedule." U.S. Patent Application No. 10/764,526.
